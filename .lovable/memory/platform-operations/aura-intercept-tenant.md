...
| Email | Role | Purpose |
|---|---|---|
| `auraintercept@gmail.com` | `platform_admin` only, no `company_id` | God-mode: manage tenants, run seeders, edit platform |
| `ai@auraintercept.ai` | `company_admin` of Aura Intercept | Run the SaaS business |
| `support@auraintercept.ai` | `employee` + `technician` | Support persona |
| `sales@auraintercept.ai` | `employee` + `technician` | Sales persona |

Tenant-account password: `aiagent*!` (reset on every reseed run).

## Re-seed
`/dashboard/demo-seeder` → "Seed Aura Intercept Tenant" (platform-admin gated). Calls edge function `seed-aura-intercept` — idempotent.
