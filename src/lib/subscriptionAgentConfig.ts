...
export const TIER_AGENT_CONFIG: Record<SubscriptionTier, TierConfig> = {
  ...
  core: {
    // Core: Talk to Aura only (no agents), Social Signal + Web Presence included
    agents: [],
    consoles: [],
    label: 'Core',
    price: '$500/mo',
    description: 'Talk to Aura + Social Signal + Web Presence',
  },
  single_point: {
  ...
  command: {
    // IMPORTANT: Keep in sync with supabase/functions/ai-agent-chat/index.ts TIER_AGENTS
    // 23 Total Agents: 4 Customer Portal + 4 Field Ops + 5 Business Ops + 3 Marketing + 3 Social Media + 4 Analytics
    agents: [
      // Customer Portal (4)
      'triage', 'booking', 'followup', 'review',
      // Field Operations (4)
