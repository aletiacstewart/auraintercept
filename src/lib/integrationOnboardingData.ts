...
export const INTEGRATION_PROVIDERS: IntegrationProvider[] = [
  ...
  },
  {
    ...
    purpose: 'Automated, scheduled multi-platform posting from the Content Engine to Facebook, Instagram, TikTok, LinkedIn, X (Twitter), YouTube, Threads, Pinterest, Reddit — up to 6 connected platforms per social set.',
    whyNeeded:
      'Copy & Post (manual paste to each network) is included on every Aura tier and requires no setup. Upload-Post is only required if the company wants Aura to schedule and publish content automatically. Each company uses its own Upload-Post account so a flag on one connected page never affects other Aura customers.',
    estTime: '20-40 minutes',
    estCost: 'From ~$9/mo (1 social set) up to ~$99/mo (multiple sets / higher volume). Billed directly by Upload-Post to the customer\'s credit card. Any per-platform ad spend is billed separately by each network.',
    signupUrl: 'https://www.upload-post.com',
    prereqs: [
      'Admin access to each business page/channel that will be connected (Facebook Page, Instagram Business, LinkedIn Company Page, TikTok Business, X account, YouTube channel, Threads, Pinterest, Reddit)',
      'Facebook Business verification completed for Instagram publishing',
      'Valid credit card ready to place on file with Upload-Post',
    ],
    steps: [
      {
        title: 'Create the Upload-Post account',
        detail: 'Go to upload-post.com -> Sign Up. Use the company\'s primary admin email (not a personal email). Verify the email address.',
      },
      {
        title: 'Choose a plan and add billing',
        detail: 'Pick the smallest plan that covers the platforms the company posts to (start at ~$9/mo for 1 social set, up to ~$99/mo). Add a valid credit card — Upload-Post will bill directly, separately from the Aura plan fee.',
      },
      {
        title: 'Create a Social Set',
        detail: 'Inside Upload-Post -> Social Sets -> New Set. A set holds up to 6 connected platforms (Facebook, Instagram, TikTok, LinkedIn, X, YouTube, Threads, Pinterest, Reddit). Name it after the brand (e.g. "{company} - Main").',
      },
      {
        title: 'Connect each social platform via OAuth',
        detail: 'For each network, click Connect and sign in as the page/channel admin. Approve the requested publish scopes. Repeat for every platform the company wants Aura to auto-post to.',
        screenshotHint: 'Upload-Post Social Set page showing all connected platforms with green "Connected" status.',
      },
      {
        title: 'Generate the Upload-Post API key',
        detail: 'Upload-Post -> Settings -> API Keys -> Create Key. Copy the key immediately (it is only shown once). This key authorizes Aura to schedule posts to the connected social set.',
      },
      {
        title: 'Paste the API key into Aura',
        detail: 'Aura -> Settings -> Integrations -> Social Media -> Upload-Post. Paste the API key and select the social set to use. Save.',
      },
      {
        title: 'Send a test post',
        detail: 'Content Engine -> New Post -> pick one platform -> Publish. Confirm the post appears on the live feed within 1-2 minutes.',
      },
    ],
    whatToPasteBack: [
      'Upload-Post API Key',
      'Social Set name / ID',
      'List of connected platforms per set',
    ],
    verification: [
      'Test post published from the Content Engine appears on each connected live feed within 2 minutes',
      'Upload-Post dashboard shows the post as "Published" (not Failed / Pending)',
      'Copy & Post (manual) continues to work on every tier even if Upload-Post is not connected',
    ],
  },
  {
    id: 'custom-domain',
