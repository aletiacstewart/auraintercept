...
export const AUDIT_FINDINGS: AuditFinding[] = [
  ...
  // ── CSS / theme ─────────────────────────────────────────────
  {
    ...
    observed:
      "Scoped passes complete: ai-consoles (7→0), components/dashboard (32→2 intentional), pages/operations (0), pages/technician (17→4 intentional), components/agents (3→3 intentional), components/marketing (22→2 intentional), components/customer-portal (0), components/ai (~75→~70 intentional), components/aura (~10→~10 intentional), components/fieldops (~13→~13 intentional), components/businessops (1→1 intentional), components/social (~10→~10 intentional), components/integrations (~30→~30 intentional), components/settings (1→0), components/onboarding (3→3 intentional), components/billing (5→2 intentional), components/calls (0), components/voice (0), components/sms (0), components/chat (3→3 intentional), components/notifications (0), components/leads (1→1), components/quotes (0), components/invoices (0), components/appointments (15→0 — AddAppointmentForm tokenized), components/customers (0), components/employee (~80→16 — fixed AppointmentCalendar+CompletedJobsHistory+JobQueue invisible text-white on default Cards), components/inventory (0), components/knowledge (13→13 intentional), components/smartwebsite (3→3 intentional), components/widget (~9→~9 intentional), components/landing (~32→~32 intentional marketing gradients), components/blog (1→1). Remaining hits are saturated gradient surfaces, status buttons, or icons on colored backgrounds — legitimate per Cyber-Sentry exception. All major /components subdirectories now swept.",
