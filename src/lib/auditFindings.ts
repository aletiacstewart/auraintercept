...
export const AUDIT_FINDINGS: AuditFinding[] = [
  ...
  // ── CSS / theme ─────────────────────────────────────────────
  {
    ...
    observed:
      "Scoped passes complete: ai-consoles (7→0), components/dashboard (32→2 intentional), pages/operations (0), pages/technician (17→4 intentional), components/agents (3→3 intentional), components/marketing (22→2 intentional), components/customer-portal (0), components/ai (~75→~70 intentional), components/aura (~10→~10 intentional), components/fieldops (~13→~13 intentional), components/businessops (1→1 intentional avatar tile), components/social (~10→~10 intentional platform mockup/badge chips), components/integrations (~30→~30 intentional numbered step badges & social platform icons). Remaining hits are saturated gradient surfaces, status buttons, or icons on colored backgrounds — legitimate per Cyber-Sentry exception. Next scopes (open): components/settings, components/onboarding, components/billing.",
