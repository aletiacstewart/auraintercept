...
/**
 * Single source of truth for the 24 Smart AI Agents = 10 consolidated
 * operatives + 14 industry specialists. This registry is consumed by
 * AgentDetailPage (settings/config UI), useAIAgentOrchestrator (default
 * agent list + category grouping), and Automation (per-agent autonomy list).
 *
 * Add or rename an agent here and it propagates to every consumer.
 * Keep in sync with:
 *   - src/lib/subscriptionAgentConfig.ts (tier gating; SPECIALIST_LABELS)
 *   - src/hooks/useAIAgentOrchestrator.ts GRANULAR_TO_OPERATIVE map
 */
...
export const AGENT_REGISTRY: Record<string, AgentRegistryEntry> = {
  ...
  triage: {
    ...
    description:
      'First point of contact that classifies customer intent and routes to appropriate specialized agents.',
  ...
  customer_journey: {
    ...
    description:
      'Manages the full post-booking relationship: scheduling, post-service follow-up, and review collection.',
    category: 'customer_engagement',
    phase: 2,
    icon: Users,
    color: 'text-cyan-400',
  ...
  dispatch: {
    ...
    capabilities: ['Smart job assignment', 'Skill matching', 'Load balancing', 'Conflict detection'],
    configFields: [
  ...
  field_navigation: {
    ...
    description:
      'Optimizes routes, tracks technician location and ETA, and manages job-site check-in/check-out.',
    category: 'field_operations',
    phase: 2,
    icon: Truck,
    color: 'text-green-500',
  ...
  business_finance: {
    ...
    description:
      'Generates quotes, handles invoicing and payment collection, and tracks parts/supply inventory.',
  ...
  admin: {
    ...
    capabilities: ['User management', 'Company settings', 'Access control', 'System configuration'],
    configFields: [
  ...
  outreach: {
    ...
    name: 'Outreach Agent',
    description:
      'Unified marketing and lead agent: promotions, referrals, win-back campaigns, seasonal outreach, and inbound lead capture/qualification.',
  ...
  creative_content: {
    ...
    description:
      'Unified AI content generation and social publishing. Creates on-brand copy, images, and posts for web, social media, campaigns, and blogs; schedules across platforms and tracks performance.',
  ...
  analytics_intelligence: {
    ...
    description:
      'Unified insights, performance, revenue, and forecasting analytics with anomaly detection and recommendations.',
  ...
  web_presence: {
    ...
    description:
      'AI website and blog management. Auto-optimizes SEO, suggests content updates, monitors site performance, and auto-publishes blog posts from the Content Engine.',
    category: 'creative_web_presence',
    phase: 2,
    icon: Globe,
...
export function getAgentEntry(type: string): AgentRegistryEntry | undefined {
  return AGENT_REGISTRY[type];
}

export function listCoreAgents(): AgentRegistryEntry[] {
  return Object.values(AGENT_REGISTRY).filter((a) => !a.isSpecialist);
}
