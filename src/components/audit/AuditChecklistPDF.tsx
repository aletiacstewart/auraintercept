...
const PlanContentsPage = ({ tier }: { tier: TierType }) => {
  ...
  return (
    <Page size="LETTER" style={styles.page}>
      ...
      <Bullet>{`60-Day Live Trial included on every plan`}</Bullet>
...
const ComparisonPage = ({ tier }: { tier: TierType }) => (
  <Page size="LETTER" style={styles.page}>
    ...
    <Text style={[styles.paragraph, { marginTop: 12, fontSize: 9, color: colors.gray }]}>
      {t('All plans include a 60-Day Live Trial and the guided launch. You can upgrade or downgrade at any time.')}
    </Text>
  </Page>
);
...
const NextStepsPage = ({ tier }: { tier: TierType }) => (
  <Page size="LETTER" style={styles.page}>
    ...
    <View style={styles.ctaBox}>
      <Text style={styles.ctaTitle}>{t('Start your 60-Day Live Trial')}</Text>
