...
const WIDGET_REGISTRY: Record<string, WidgetSpec> = {
  ...
  // Cluster B/C/D — placeholders (coming in Phases 3-5)
  recurring_route_map: { label: 'Recurring Route Map', icon: Map, description: 'Weekly/monthly route clusters.', comingSoon: true },
  storm_map: { label: 'Storm Tracker', icon: Activity, description: 'Local storm damage windows.', comingSoon: true },
  insurance_claim_tracker: { label: 'Insurance Claims', icon: FileText, description: 'Open and pending claims.', comingSoon: true },
  site_survey_queue: { label: 'Site Surveys', icon: Map, description: 'Pre-install surveys scheduled.', comingSoon: true },
  production_monitor: { label: 'Solar Production', icon: Sun, description: 'kWh produced per system.', comingSoon: true },
  incentive_calculator: { label: 'Incentives', icon: FileText, description: 'Local rebates + tax credits.', comingSoon: true },
  seasonal_calendar: { label: 'Seasonal Calendar', icon: Clock, description: 'Service windows by season.', comingSoon: true },
  crew_scheduler: { label: 'Crew Scheduler', icon: Clock, description: 'Multi-tech crew assignments.', comingSoon: true },
  equipment_tracker: { label: 'Equipment Status', icon: Wrench, description: 'Mowers, trailers, equipment.', comingSoon: true },
  weather_alerts: { label: 'Weather Alerts', icon: AlertCircle, description: 'Reschedule triggers from weather.', comingSoon: true },
  material_calculator: { label: 'Material Calculator', icon: Wrench, description: 'Linear ft, sq ft, material lists.', comingSoon: true },
  quote_pipeline: { label: 'Quote Pipeline', icon: FileText, description: 'Open quotes by stage.', comingSoon: true },
  chemistry_log: { label: 'Pool Chemistry', icon: Waves, description: 'pH, chlorine, alkalinity by stop.', comingSoon: true },
  infestation_map: { label: 'Infestation Map', icon: Bug, description: 'Active treatments by area.', comingSoon: true },
  treatment_log: { label: 'Treatment Log', icon: FileText, description: 'Per-customer treatment history.', comingSoon: true },
  bay_scheduler: { label: 'Bay Scheduler', icon: Building2, description: 'Auto bay assignments.', comingSoon: true },
  vin_lookup: { label: 'VIN Lookup', icon: FileText, description: 'Vehicle history by VIN.', comingSoon: true },
  service_history: { label: 'Service History', icon: Clock, description: 'Per-vehicle service records.', comingSoon: true },
  task_bundling: { label: 'Task Bundling', icon: Hammer, description: 'Combine small tasks into one visit.', comingSoon: true },
  multi_phase_tracker: { label: 'Project Phases', icon: Activity, description: 'Multi-phase construction tracker.', comingSoon: true },
  monitoring_status: { label: 'Monitoring Status', icon: Shield, description: 'Active alarm-monitoring sessions.', comingSoon: true },
  installation_queue: { label: 'Install Queue', icon: Wrench, description: 'Upcoming security installs.', comingSoon: true },
