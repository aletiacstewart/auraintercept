...
interface BlogBatchWizardProps {
  open: boolean;
...
const WORD_COUNT_OPTIONS = [
  { value: '500', label: '500 words (Quick read)' },
  { value: '1000', label: '1000 words (Standard)' },
  { value: '1500', label: '1500 words (In-depth)' },
  { value: '2000', label: '2000 words (Comprehensive)' },
];

export function BlogBatchWizard({ open, onOpenChange, onSuccess }: BlogBatchWizardProps) {
  ...
  // Update single topic
  const updateTopic = (id: string, field: keyof TopicEntry, value: string | Date) => {
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (hasTavily) {
        setGenerationPhase('Researching industry trends...');
        setGenerationProgress(20);
      }

      setGenerationPhase(`Generating ${validTopics.length} blog posts...`);
    ...
    onSuccess: (data) => {
      toast.success(`Generated ${data.generatedCount} blog posts for approval!`);
  ...
  const getStepTitle = () => {
    switch (step) {
      case 1: return 'Schedule Settings';
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2 col-span-2">
                <Label>Word Count per Post</Label>
          ...
          <div className="flex flex-col items-center justify-center py-12 space-y-6">
            <div className="relative">
              ...
              {hasTavily && generationPhase.includes('Research') && (
                <Search className="h-6 w-6 absolute -top-1 -right-1 text-primary animate-pulse" />
              )}
            </div>
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {hasTavily 
                  ? 'AI is researching trends and generating your blog posts...'
                  : 'AI is generating your blog posts...'}
              </p>
            </div>
