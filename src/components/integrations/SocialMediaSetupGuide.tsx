...
const PLATFORM_CONFIG = {
  facebook: {
    ...
    steps: [
      ...
      {
        title: '⚙️ Configure Webhooks — Messenger Use Case (Step 1)',
        content: [
          ...
          'Set Verify Token to a secret string you choose (e.g., "aura_verify_2024") — save it securely',
          'Click "Verify and save" — Meta will send a GET request to verify your endpoint',
          'Store your Verify Token as META_WEBHOOK_VERIFY_TOKEN in the integration secrets',
      ...
      {
        title: '⚙️ Configure Webhooks — Global Webhooks Page',
        content: [
          'Go to your Meta App → Webhooks (left sidebar)',
          'In the "Select product" dropdown, choose "Page"',
          'Click "Subscribe to this object" (or "Edit subscription" if already configured)',
          'Paste the same Callback URL and Verify Token you used above',
          'Click "Verify and save"',
          'Now subscribe to the fields below by clicking "Subscribe" next to each:',
          ...
          '⬜ ratings — Page ratings (optional)',
          'Leave all other fields (affiliation, attire, parking, culinary_team, etc.) UNSUBSCRIBED',
          'Important: Use the same API version (v24.0) for every subscribed field',
        ],
        urls: [
          { label: 'Webhook Callback URL', urlKey: 'webhook' as const },
        ],
      },
      {
        ...
        content: [
          'In Messenger settings, expand "2. Generate access tokens"',
          'You will see a table with columns: Page Name, Webhook Subscription, Token',
          'Click "Add Page" at the bottom to connect your Facebook Page',
          'Select your Page (e.g., "Aura Intercept") from the dialog',
          'Once added, click "Add Subscriptions" next to your page',
          'Select the same fields: feed, messages, messaging_postbacks, messaging_optins',
          'Click "Generate" next to your page to create a Page Access Token',
          'IMPORTANT: This generates a short-lived token — exchange it for a long-lived one:',
          'Use: GET /oauth/access_token?grant_type=fb_exchange_token&client_id=APP_ID&client_secret=APP_SECRET&fb_exchange_token=SHORT_LIVED_TOKEN',
          'Store the long-lived Page Access Token in your integration settings on this page',
        ],
      },
      {
