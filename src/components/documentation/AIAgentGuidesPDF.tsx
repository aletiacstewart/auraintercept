...
const TIERS = [
  {
    name: SUBSCRIPTION_TIERS.aura_connect?.name ?? 'Aura Connect',
    price: `$${(SUBSCRIPTION_TIERS.aura_connect?.price ?? 297).toLocaleString()}/month`,
    annualPrice: `$${(SUBSCRIPTION_TIERS.aura_connect?.annualPrice ?? 2970).toLocaleString()}/year`,
    consoles: [`${SUBSCRIPTION_TIERS.aura_connect?.consoles ?? 4} Consoles`],
    agentCount: SUBSCRIPTION_TIERS.aura_connect?.operatives ?? 5,
    agents: ['AI Receptionist (Triage)', 'Customer Journey Agent', 'Outreach Agent', 'Creative Content Agent', 'Web Presence Agent'],
    voiceIncluded: SUBSCRIPTION_TIERS.aura_connect?.hasVoice ?? true,
    note: SUBSCRIPTION_TIERS.aura_connect?.description ?? 'Core AI operatives for booking, outreach, and web presence.',
  },
  {
    name: SUBSCRIPTION_TIERS.multi_track?.name ?? 'Aura Performance',
    price: `$${(SUBSCRIPTION_TIERS.multi_track?.price ?? 497).toLocaleString()}/month`,
    annualPrice: `$${(SUBSCRIPTION_TIERS.multi_track?.annualPrice ?? 4970).toLocaleString()}/year`,
    consoles: [`Up to ${SUBSCRIPTION_TIERS.multi_track?.consoles ?? 6} Consoles`],
    agentCount: SUBSCRIPTION_TIERS.multi_track?.operatives ?? 7,
    agents: [
      'AI Receptionist', 'Customer Journey', 'Outreach', 'Creative Content',
      'Web Presence', 'Dispatch Agent', 'Field Navigation Agent',
    ],
    voiceIncluded: SUBSCRIPTION_TIERS.multi_track?.hasVoice ?? true,
    note: SUBSCRIPTION_TIERS.multi_track?.description ?? 'Adds field ops: dispatch, routing, and navigation agents.',
  },
  {
    name: SUBSCRIPTION_TIERS.command?.name ?? 'Aura Command',
    price: `$${(SUBSCRIPTION_TIERS.command?.price ?? 697).toLocaleString()}/month`,
    annualPrice: `$${(SUBSCRIPTION_TIERS.command?.annualPrice ?? 6970).toLocaleString()}/year`,
    consoles: [`All ${SUBSCRIPTION_TIERS.command?.consoles ?? 7} Consoles`],
    agentCount: SUBSCRIPTION_TIERS.command?.operatives ?? 10,
    agents: [`All ${SUBSCRIPTION_TIERS.command?.operatives ?? 10} AI Operatives`],
    voiceIncluded: SUBSCRIPTION_TIERS.command?.hasVoice ?? true,
    note: SUBSCRIPTION_TIERS.command?.description ?? 'Full 10-operative suite with analytics, admin, and business finance.',
  },
];
